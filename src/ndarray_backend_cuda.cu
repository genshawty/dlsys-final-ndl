...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  for (uint32_t i = 0; i < shape.size; i++) {
            pos += (num % shape.data[shape.size - 1 - i]) * strides.data[strides.size - i - 1];
            num = num / shape.data[shape.size - 1 - i];
        };
...
__global__ void MatMulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size, uint32_t m, uint32_t n, uint32_t p) {
  ...
  auto res = a[row * n] * b[col];

  for (size_t i = 1; i < n; i++) {
    res = res + a[row * n + i] * b[col + i * p];
  };
  out[gid] = res;
}
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  auto m = a[i * reduce_size];
  for (size_t red = 0; red < reduce_size; red++) {
    if (a[i * reduce_size + red] > m) {
      m = a[i * reduce_size + red];
    }
  };
  out[gid] = m;
}
...
__global__ void ReduceSumKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
  ...
  auto sum = a[i * reduce_size];
  for (size_t red = 1; red < reduce_size; red++) {
      sum = sum + a[i * reduce_size + red];
  };
  out[gid] = sum;
}
