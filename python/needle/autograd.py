...
class Tensor(Value):
    ...
    def mean(self, axes=None, keepdims=False):
        return needle.ops.Mean(axes, keepdims)(self)
 
    def var(self, axes=None, keepdims=False):
        return needle.ops.Variation(axes, keepdims)(self)
    ...
    def square(self):
        return needle.ops.square(self)
 
    def sqrt(self):
        return needle.ops.sqrt(self)

    # def var(self, axes=None, keepdims=False):
    #     return needle.ops.Var(axes=axes, keepdims=keepdims)(self)
    
    def mean(self, axes=None, keepdims=False):
        return needle.ops.Mean(axes=axes, keepdims=keepdims)(self)
...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    #             shapes = (node.inputs[0].shape, node.inputs[1].shape)
    #         else:
    #             shapes = (node.inputs[0].shape)
    #     print("node: ", node.shape, "op: ", node.op, "inputs: ", *shapes)
    ...
    for node in reverse_topo_order:
        try:
            ...
            for i, subnode in enumerate(node.inputs):
                if node_to_output_grads_list.get(subnode):
                    node_to_output_grads_list[subnode].append(gradients[i])
                else:
                    node_to_output_grads_list[subnode] = [gradients[i]]
            # for i in node_to_output_grads_list.keys():
            #     print(i.op, sum_node_list(node_to_output_grads_list[i]))
            # print("====================================")
        except Exception as e:
            for i in node_to_output_grads_list.keys():
                print("op: ", i.op, "grad: ",sum_node_list(node_to_output_grads_list[i]).shape, end="inputs: ")
                for inp in i.inputs:
                    print(inp.shape, end=" ")
                print()
            print("================================")  
            print("op: ", node.op, "grad: ",sum_node_list(node_to_output_grads_list[node]).shape, end="inputs: ")
...
def find_topo_sort(node_list: List[Value]) -> List[Value]:
    ...
    sort.
    """
    topo_sort = topo_sort_dfs(node_list[0], set(), list())[::-1]
    return topo_sort
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    if id(node) in visited:
        topo_order.pop(topo_order.index(node))
    visited.add(id(node))
    topo_order.append(node)

    if node.inputs:
        for subnode in node.inputs[::-1]:
