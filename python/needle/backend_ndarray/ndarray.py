...
class NDArray:
    ...
    def reshape(self, *args):
        ...
        if len(new_shape) == 1:
            ...
            new_strides = [None]*len(new_shape)
            new_strides[-1] = 1
    ...
    def permute(self, *new_axes):
        ...
        Returns:
            ...
            strides changed).
        """
        if isinstance(new_axes, tuple) or isinstance(new_axes, list):
            new_axes = new_axes[0]
        ...
        for i in range(len(new_axes)):
            ...
            new_stride[i] = self.strides[new_axes[i]]
    ...
    def broadcast_to(self, new_shape):
        ...
        def get_shape_change(old: list, new: list):
            ret = [1]*len(new)
        ...
        mask = get_shape_change(self.shape, new_shape)

        new_strides = [0]*len(mask)
        k = 0
        ...
        for i in range(len(mask)):
            if mask[i]:
                ...
                new_strides[i] = self.strides[i - k]
        
        new_strides = tuple(new_strides)
        print("PRINTING STUFF")
        print("prev strides:", self.strides)
        print("new strides", new_strides)
        print("prev_shape", self.shape)
    ...
    def __getitem__(self, idxs):
        ...
        for i in range(len(idxs)):
            id = idxs[i]
            new_shape[i] = (1 + (id.stop - id.start - 1) // id.step)
        ...
        for i in range(len(idxs)):
            id = idxs[i]
            offset += id.start * new_strides[i]
            new_strides[i] = id.step * new_strides[i]
        
        # print(self)
        # print(idxs)
        # print("old strides: ", self.strides)
        # print("new strides: ", new_strides)
        ...
        return self.as_strided(tuple(new_shape), tuple(new_strides), offset=offset)
    ...
    def flip(self, axes):
        ...
        for i in range(len(new_strides)):
            if i in axes:
                ...
                offset += self.strides[i] * (self.shape[i] -1)
            
        out = NDArray.make(self.shape, new_strides, device=self.device, handle=self._handle, offset=offset)
        out = out.compact()
        return out
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        out = NDArray.make(shape=new_shape, strides=self.strides, device=self.device, offset=self._offset)
        out.fill(0)
        
        slice_arr = []
        for i in range(len(new_shape)):
            slice_arr.append(slice(axes[i][0], new_shape[i] - axes[i][1], 1))
        
        out2 = out.numpy()
        out2[tuple(slice_arr)] = self.numpy()
        
        return NDArray(out2, device=self.device)

def pad(array, axes):
    return array.pad(axes)

def transpose(array, axes):
    new_order = list(range(len(array.shape)))
    axes = list(axes)
    # print("AXES TO SWAP", axes)
    for i, s in enumerate(axes):
        if s < 0:
            axes[i] += len(array.shape)

    if len(axes) != 2:
        raise NotImplementedError()

    temp = new_order[axes[0]]
    new_order[axes[0]] = new_order[axes[1]]
    new_order[axes[1]] = temp

    # print("AXES TO SWAP", axes)
    # print("AFTER PERMUTE", new_order)

    return array.permute(tuple(new_order))
